...
bool ElectronFilterPandora::opticalfilter(size_t ipf, const std::vector<recob::PFParticle> & pfparticles,const art::Event & evt)
{
    ...
    //std::cout << std::endl;
    //std::cout << "Number of flashes in this event: " << optical_vec.size() << std::endl;
    //std::cout << "The charge centre of PFP " << ipf << " is: (" << chargecenter[0] << "," << chargecenter[1] << "," << chargecenter[2] << ")" << std::endl;
...
bool ElectronFilterPandora::filter(art::Event & evt)
{
    ...
    try
    {
        ...
        for (size_t ipf = 0; ipf < pfparticles.size(); ipf++)
        {
            ...
            if(!opticalfilter(ipf,pfparticles,evt)) continue;

            int showers = 0;
            int tracks = 0;

            if(!ElectronFilterPandora::opticalfilter(ipf,pfparticles,evt)) continue;
